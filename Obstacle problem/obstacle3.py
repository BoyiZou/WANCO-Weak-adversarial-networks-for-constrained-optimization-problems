...
def phi_fun(x):
    ...
    phi = 10*torch.sin(math.pi*(x+1)**2)**2
...
def exactnew(data,params):
    ...
    nexact1 = (( 0.204241787711896 -xx)>0+0).reshape(-1,1)*( (9.757203835481757-5)/( 0.204241787711896) *xx + 5)

    nexact2 = (( 0.204241787711896 -xx)*(xx-(1.5**0.5-1))>0+0).reshape(-1,1)*10*torch.sin(math.pi*(xx+1)**2)**2

    nexact3 = (( (1.5**0.5-1) -xx)*(xx-1.1)>0+0).reshape(-1,1)*10
...
def main():
    ...
    params["milestone"] = [1500,3500,6000,8000,13000,14500,16000,19000,21000,23000,25000,27000,29000,35000,39000,42000,45000,48000,49500]
