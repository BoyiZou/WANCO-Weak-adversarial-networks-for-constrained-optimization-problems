...
def trainWANCO(model,device,params,optimizer,scheduler,model_2,optimizer_2,scheduler_2):
    ...
    MM = open("mass_u7.txt","w")
    NN = open("mass_u8.txt","w")
    LL = open("mass_u9.txt","w")
    KK = open("beta.txt","w")
    ...
    for step in range(params["trainStep"]):
        ...
        du8dxy = torch.autograd.grad(u8,data,grad_outputs=torch.ones_like(u8),retain_graph=True,create_graph=True,only_inputs=True,allow_unused=True)[0]
        du9dxy = torch.autograd.grad(u9,data,grad_outputs=torch.ones_like(u9),retain_graph=True,create_graph=True,only_inputs=True,allow_unused=True)[0]
        ...
        grad_u8_2norm_2 = du8dxy[:,0]**2+du8dxy[:,1]**2
        grad_u8_2norm_2 = grad_u8_2norm_2.reshape(-1,1)
        grad_u9_2norm_2 = du9dxy[:,0]**2+du9dxy[:,1]**2
        grad_u9_2norm_2 = grad_u9_2norm_2.reshape(-1,1)
        ...
        mass_u8 = (torch.mean(u8**2)-params["volumn"])
        mass_u9 = (torch.mean(u9**2)-params["volumn"])
        mass = torch.abs(mass_u1) + torch.abs(mass_u2) + torch.abs(mass_u3) + torch.abs(mass_u4) + torch.abs(mass_u5) + torch.abs(mass_u6) + torch.abs(mass_u7) + torch.abs(mass_u8) + torch.abs(mass_u9)

        loss = params["penalty2"]*(1/2*(torch.mean(grad_u1_2norm_2)+torch.mean(grad_u2_2norm_2)+torch.mean(grad_u3_2norm_2)+torch.mean(grad_u4_2norm_2)+torch.mean(grad_u5_2norm_2)+torch.mean(grad_u6_2norm_2)+torch.mean(grad_u7_2norm_2)+torch.mean(grad_u8_2norm_2)+torch.mean(grad_u9_2norm_2))\
                                   + (2/(params["epsilon"]**2))*torch.mean((u1*u2)**2+(u1*u3)**2+(u1*u4)**2+(u1*u5)**2+(u1*u6)**2+(u1*u7)**2+(u1*u8)**2+(u1*u9)**2\
                                   +(u2*u3)**2+(u2*u4)**2+(u2*u5)**2+(u2*u6)**2+(u2*u7)**2+(u2*u8)**2+(u2*u9)**2+(u3*u4)**2+(u3*u5)**2+(u3*u6)**2+(u3*u7)**2+(u3*u8)**2+(u3*u9)**2\
                                   +(u4*u5)**2+(u4*u6)**2+(u4*u7)**2+(u4*u8)**2+(u4*u9)**2+(u5*u6)**2+(u5*u7)**2+(u5*u8)**2+(u5*u9)**2+(u6*u7)**2+(u6*u8)**2+(u6*u9)**2+(u7*u8)**2+(u7*u9)**2+(u8*u9)**2))\
                                   -  (yyy_2[0,:]*mass_u1+yyy_2[1,:]*mass_u2+yyy_2[2,:]*mass_u3+yyy_2[3,:]*mass_u4+yyy_2[4,:]*mass_u5+yyy_2[5,:]*mass_u6+yyy_2[6,:]*mass_u7+yyy_2[7,:]*mass_u8+yyy_2[8,:]*mass_u9)\
                                   +  params["penalty"]*(mass_u1**2+mass_u2**2+mass_u3**2+mass_u4**2+mass_u5**2+mass_u6**2+mass_u7**2+mass_u8**2+mass_u9**2)

        loss_2 =  (yyy_2[0,:]*mass_u1+yyy_2[1,:]*mass_u2+yyy_2[2,:]*mass_u3+yyy_2[3,:]*mass_u4+yyy_2[4,:]*mass_u5+yyy_2[5,:]*mass_u6+yyy_2[6,:]*mass_u7+yyy_2[7,:]*mass_u8+yyy_2[8,:]*mass_u9)
        ...
        if step%params["writeStep"] == params["writeStep"]-1:
          ...
          file = open("mass_u8.txt","a")
          file.write(str(mass_u8.cpu().detach().numpy())+"\n")
          file = open("mass_u9.txt","a")
          file.write(str(mass_u9.cpu().detach().numpy())+"\n")
        ...
        if step%params["plotStep"] == params["plotStep"]-1:
          ...
          Train_u8 = Train_u[:,7].reshape(Pix+1,Pix+1)
          Train_u9 = Train_u[:,8].reshape(Pix+1,Pix+1)
          plt.figure()
          ...
          plt.imshow(Train_u[:,7].reshape(Pix+1,Pix+1),cmap = 'viridis',origin='lower')
          cb = plt.colorbar(shrink=0.7)
          plt.show()

          plt.imshow(Train_u[:,8].reshape(Pix+1,Pix+1),cmap = 'viridis',origin='lower')
          cb = plt.colorbar(shrink=0.7)
          plt.show()
          ...
          plt.imshow(Train_u[:,1].reshape(Pix+1,Pix+1)+Train_u[:,0].reshape(Pix+1,Pix+1)+Train_u[:,2].reshape(Pix+1,Pix+1)\
                     ...
                     +Train_u[:,6].reshape(Pix+1,Pix+1)+Train_u[:,7].reshape(Pix+1,Pix+1)+Train_u[:,8].reshape(Pix+1,Pix+1),cmap = 'viridis',origin='lower')
          cb = plt.colorbar(shrink=0.7)
          plt.show()
          ...
          a18 = (Train_u1>Train_u8)+0
          a19 = (Train_u1>Train_u9)+0
          ...
          b28 = (Train_u2>Train_u8)+0
          b29 = (Train_u2>Train_u9)+0
          ...
          c38 = (Train_u3>Train_u8)+0
          c39 = (Train_u3>Train_u9)+0
          ...
          d48 = (Train_u4>Train_u8)+0
          d49 = (Train_u4>Train_u9)+0
          ...
          e58 = (Train_u5>Train_u8)+0
          e59 = (Train_u5>Train_u9)+0
          ...
          f68 = (Train_u6>Train_u8)+0
          f69 = (Train_u6>Train_u9)+0
          ...
          g78 = (Train_u7>Train_u8)+0
          g79 = (Train_u7>Train_u9)+0

          h81 = (Train_u8>Train_u1)+0
          h82 = (Train_u8>Train_u2)+0
          h83 = (Train_u8>Train_u3)+0
          h84 = (Train_u8>Train_u4)+0
          h85 = (Train_u8>Train_u5)+0
          h86 = (Train_u8>Train_u6)+0
          h87 = (Train_u8>Train_u7)+0
          h89 = (Train_u8>Train_u9)+0

          i91 = (Train_u9>Train_u1)+0
          i92 = (Train_u9>Train_u2)+0
          i93 = (Train_u9>Train_u3)+0
          i94 = (Train_u9>Train_u4)+0
          i95 = (Train_u9>Train_u5)+0
          i96 = (Train_u9>Train_u6)+0
          i97 = (Train_u9>Train_u7)+0
          i98 = (Train_u9>Train_u8)+0
          ...
          a18 = torch.from_numpy(a18)
          a19 = torch.from_numpy(a19)
          ...
          b28 = torch.from_numpy(b28)
          b29 = torch.from_numpy(b29)
          ...
          c38 = torch.from_numpy(c38)
          c39 = torch.from_numpy(c39)
          ...
          d48 = torch.from_numpy(d48)
          d49 = torch.from_numpy(d49)
          ...
          e58 = torch.from_numpy(e58)
          e59 = torch.from_numpy(e59)
          ...
          f68 = torch.from_numpy(f68)
          f69 = torch.from_numpy(f69)
          ...
          g78 = torch.from_numpy(g78)
          g79 = torch.from_numpy(g79)

          h81 = torch.from_numpy(h81)
          h82 = torch.from_numpy(h82)
          h83 = torch.from_numpy(h83)
          h84 = torch.from_numpy(h84)
          h85 = torch.from_numpy(h85)
          h86 = torch.from_numpy(h86)
          h87 = torch.from_numpy(h87)
          h89 = torch.from_numpy(h89)

          i91 = torch.from_numpy(i91)
          i92 = torch.from_numpy(i92)
          i93 = torch.from_numpy(i93)
          i94 = torch.from_numpy(i94)
          i95 = torch.from_numpy(i95)
          i96 = torch.from_numpy(i96)
          i97 = torch.from_numpy(i97)
          i98 = torch.from_numpy(i98)

          phase_u1 = 1*F.relu(a12+a13+a14+a15+a16+a17+a18+a19-7)
          phase_u2 = 2*F.relu(b21+b23+b24+b25+b26+b27+b28+b29-7)
          phase_u3 = 3*F.relu(c31+c32+c34+c35+c36+c37+c38+c39-7)
          phase_u4 = 4*F.relu(d41+d42+d43+d45+d46+d47+d48+d49-7)
          phase_u5 = 5*F.relu(e51+e52+e53+e54+e56+e57+e58+e59-7)
          phase_u6 = 6*F.relu(f61+f62+f63+f64+f65+f67+f68+f69-7)
          phase_u7 = 7*F.relu(g71+g72+g73+g74+g75+g76+g78+g79-7)
          phase_u8 = 8*F.relu(h81+h82+h83+h84+h85+h86+h87+h89-7)
          phase_u9 = 9*F.relu(i91+i92+i93+i94+i95+i96+i97+i98-7)

          phase = phase_u1+phase_u2+phase_u3+phase_u4+phase_u5+phase_u6+phase_u7+phase_u8+phase_u9
