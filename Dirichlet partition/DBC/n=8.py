...
def trainWANCO(model,device,params,optimizer,scheduler,model_2,optimizer_2,scheduler_2):
    ...
    for step in range(params["trainStep"]):
        ...
        mass = torch.abs(mass_u1) + torch.abs(mass_u2) + torch.abs(mass_u3) + torch.abs(mass_u4) + torch.abs(mass_u5) + torch.abs(mass_u6) + torch.abs(mass_u7) + torch.abs(mass_u8)

        loss = params["penalty2"]*(1/2*(torch.mean(grad_u1_2norm_2)+torch.mean(grad_u2_2norm_2)+torch.mean(grad_u3_2norm_2)+torch.mean(grad_u4_2norm_2)+torch.mean(grad_u5_2norm_2)+torch.mean(grad_u6_2norm_2)+torch.mean(grad_u7_2norm_2)+torch.mean(grad_u8_2norm_2))\
                                   + (2/(params["epsilon"]**2))*torch.mean((u1*u2)**2+(u1*u3)**2+(u1*u4)**2+(u1*u5)**2+(u1*u6)**2+(u1*u7)**2+(u1*u8)**2\
                                   +(u2*u3)**2+(u2*u4)**2+(u2*u5)**2+(u2*u6)**2+(u2*u7)**2+(u2*u8)**2+(u3*u4)**2+(u3*u5)**2+(u3*u6)**2+(u3*u7)**2+(u3*u8)**2\
                                   +(u4*u5)**2+(u4*u6)**2+(u4*u7)**2+(u4*u8)**2+(u5*u6)**2+(u5*u7)**2+(u5*u8)**2+(u6*u7)**2+(u6*u8)**2+(u7*u8)**2))\
                                   - yyy_2*(mass_u1+mass_u2+mass_u3+mass_u4+mass_u5+mass_u6+mass_u7+mass_u8)\
                                   +  params["penalty"]*(mass_u1**2+mass_u2**2+mass_u3**2+mass_u4**2+mass_u5**2+mass_u6**2+mass_u7**2+mass_u8**2)

        loss_2 =  yyy_2*(mass_u1+mass_u2+mass_u3+mass_u4+mass_u5+mass_u6+mass_u7+mass_u8)
        ...
        if step%params["plotStep"] == params["plotStep"]-1:
          ...
          plt.imshow(Train_u[:,1].reshape(Pix+1,Pix+1)+Train_u[:,0].reshape(Pix+1,Pix+1)+Train_u[:,2].reshape(Pix+1,Pix+1)\
                     ...
                     +Train_u[:,6].reshape(Pix+1,Pix+1)+Train_u[:,7].reshape(Pix+1,Pix+1),cmap = 'viridis',origin='lower')
          cb = plt.colorbar(shrink=0.7)
          plt.show()
          ...
          phase_u1 = 1*F.relu(a12+a13+a14+a15+a16+a17+a18-6)
          phase_u2 = 2*F.relu(b21+b23+b24+b25+b26+b27+b28-6)
          phase_u3 = 3*F.relu(c31+c32+c34+c35+c36+c37+c38-6)
          phase_u4 = 4*F.relu(d41+d42+d43+d45+d46+d47+d48-6)
          phase_u5 = 5*F.relu(e51+e52+e53+e54+e56+e57+e58-6)
          phase_u6 = 6*F.relu(f61+f62+f63+f64+f65+f67+f68-6)
          phase_u7 = 7*F.relu(g71+g72+g73+g74+g75+g76+g78-6)
          phase_u8 = 8*F.relu(h81+h82+h83+h84+h85+h86+h87-6)


          phase = phase_u1+phase_u2+phase_u3+phase_u4+phase_u5+phase_u6+phase_u7+phase_u8
