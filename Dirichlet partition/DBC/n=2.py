...
def trainWANCO(model,device,params,optimizer,scheduler,model_2,optimizer_2,scheduler_2):
    ...
    for step in range(params["trainStep"]):
        ...
        mass = torch.abs(mass_u1) + torch.abs(mass_u2)

        loss = params["penalty2"]*(1/2*(torch.mean(grad_u1_2norm_2)+torch.mean(grad_u2_2norm_2))\
                                   + (2/(params["epsilon"]**2))*torch.mean((u1*u2)**2))\
                                   - yyy_2*(mass_u1+mass_u2)\
                                   +  params["penalty"]*(mass_u1**2+mass_u2**2)

        loss_2 =  yyy_2*(mass_u1+mass_u2)
        ...
        if step%params["plotStep"] == params["plotStep"]-1:
          ...
          plt.figure()
          plt.imshow(Train_u[:,1].reshape(Pix+1,Pix+1)+Train_u[:,0].reshape(Pix+1,Pix+1),cmap = 'viridis',origin='lower')
          cb = plt.colorbar(shrink=0.7)
          plt.show()
...
def main():
    ...
    params["penalty"] = 10000000  ## beta in paper
    params["alpha"] = 1.0005
    params["volumn"] = 1
