...
def trainWANCO(model,device,params,optimizer,scheduler,model_2,optimizer_2,scheduler_2):
    ...
    for step in range(params["trainStep"]):
        ...
        mass = torch.abs(mass_u1) + torch.abs(mass_u2) + torch.abs(mass_u3) + torch.abs(mass_u4) + torch.abs(mass_u5)

        loss = params["penalty2"]*(1/2*(torch.mean(grad_u1_2norm_2)+torch.mean(grad_u2_2norm_2)+torch.mean(grad_u3_2norm_2)+torch.mean(grad_u4_2norm_2)+torch.mean(grad_u5_2norm_2))\
                                   + (2/(params["epsilon"]**2))*torch.mean((u1*u2)**2+(u1*u3)**2+(u1*u4)**2+(u1*u5)**2+(u2*u3)**2+(u2*u4)**2+(u2*u4)**2+(u2*u5)**2+(u3*u4)**2+(u3*u5)**2+(u4*u5)**2))\
                                   - yyy_2*(mass_u1+mass_u2+mass_u3+mass_u4+mass_u5)\
                                   +  params["penalty"]*(mass_u1**2+mass_u2**2+mass_u3**2+mass_u4**2+mass_u5**2)

        loss_2 =  yyy_2*(mass_u1+mass_u2+mass_u3+mass_u4+mass_u5)
        ...
        if step%params["plotStep"] == params["plotStep"]-1:
          ...
          plt.figure()
          plt.imshow(Train_u[:,1].reshape(Pix+1,Pix+1)+Train_u[:,0].reshape(Pix+1,Pix+1)+Train_u[:,2].reshape(Pix+1,Pix+1)+Train_u[:,3].reshape(Pix+1,Pix+1)+Train_u[:,4].reshape(Pix+1,Pix+1),cmap = 'viridis',origin='lower')
          cb = plt.colorbar(shrink=0.7)
          plt.show()
          ...
          eee = (Train_u2>Train_u1)+0
          fff = (Train_u2>Train_u3)+0
          ggg = (Train_u2>Train_u4)+0
          hhh = (Train_u2>Train_u5)+0

          iii = (Train_u3>Train_u1)+0
          jjj = (Train_u3>Train_u2)+0
          kkk = (Train_u3>Train_u4)+0
          lll = (Train_u3>Train_u5)+0

          mmm = (Train_u4>Train_u1)+0
          nnn = (Train_u4>Train_u2)+0
          ooo = (Train_u4>Train_u3)+0
          ppp = (Train_u4>Train_u5)+0

          qqq = (Train_u5>Train_u1)+0
          rrr = (Train_u5>Train_u2)+0
          sss = (Train_u5>Train_u3)+0
          ttt = (Train_u5>Train_u4)+0
          ...
          phase_u1 = 1*F.relu(aaa+bbb+ccc+ddd-3)
          phase_u2 = 2*F.relu(eee+fff+ggg+hhh-3)
          phase_u3 = 3*F.relu(iii+jjj+kkk+lll-3)
          phase_u4 = 4*F.relu(mmm+nnn+ooo+ppp-3)
          phase_u5 = 5*F.relu(qqq+rrr+sss+ttt-3)
          phase = phase_u1+phase_u2+phase_u3+phase_u4+phase_u5
