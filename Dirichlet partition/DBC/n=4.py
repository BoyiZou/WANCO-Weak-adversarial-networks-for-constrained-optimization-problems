...
def trainWANCO(model,device,params,optimizer,scheduler,model_2,optimizer_2,scheduler_2):
    ...
    for step in range(params["trainStep"]):
        ...
        mass = torch.abs(mass_u1) + torch.abs(mass_u2) + torch.abs(mass_u3) + torch.abs(mass_u4)

        loss = params["penalty2"]*(1/2*(torch.mean(grad_u1_2norm_2)+torch.mean(grad_u2_2norm_2)+torch.mean(grad_u3_2norm_2)+torch.mean(grad_u4_2norm_2)) + (3/(params["epsilon"]**2))*torch.mean((u1*u2)**2+(u1*u3)**2+(u1*u4)**2+(u2*u3)**2+(u2*u4)**2+(u3*u4)**2)) - yyy_2*(mass_u1+mass_u2+mass_u3+mass_u4)  +  params["penalty"]*(mass_u1**2+mass_u2**2+mass_u3**2+mass_u4**2)

        loss_2 =  yyy_2*(mass_u1+mass_u2+mass_u3+mass_u4)
        ...
        if step%params["plotStep"] == params["plotStep"]-1:
          ...
          plt.figure()
          plt.imshow(Train_u[:,1].reshape(Pix+1,Pix+1)+Train_u[:,0].reshape(Pix+1,Pix+1)+Train_u[:,2].reshape(Pix+1,Pix+1)+Train_u[:,3].reshape(Pix+1,Pix+1),cmap = 'viridis',origin='lower')
          cb = plt.colorbar(shrink=0.7)
          plt.show()
          ...
          ddd = (Train_u2>Train_u1)+0
          eee = (Train_u2>Train_u3)+0
          fff = (Train_u2>Train_u4)+0
          ggg = (Train_u3>Train_u1)+0
          hhh = (Train_u3>Train_u2)+0
          iii = (Train_u3>Train_u4)+0
          jjj = (Train_u4>Train_u1)+0
          kkk = (Train_u4>Train_u2)+0
          lll = (Train_u4>Train_u3)+0
          ...
          phase_u1 = 1*F.relu(aaa+bbb+ccc-2)
          phase_u2 = 2*F.relu(ddd+eee+fff-2)
          phase_u3 = 3*F.relu(ggg+hhh+iii-2)
          phase_u4 = 4*F.relu(jjj+kkk+lll-2)

          phase = phase_u1+phase_u2+phase_u3+phase_u4
