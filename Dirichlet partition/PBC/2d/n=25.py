...
class ResNet(torch.nn.Module):
    def __init__(self, params, device):
        ...
        self.params = params
        self.device = device
        # self.linearIn = nn.Linear(self.params["d"], self.params["width"]) # the input layer is omitted since we enforce exact periodicity and pad with zeros 
    ...
    def forward(self, x):
        ...
        X_1 = x[:,0]
        X_2 = x[:,1] # record the input coordinates to exactly imposed the boundary (efficient for simple geometry)
        a = x.size()[0]
        ...
        x = F.relu(x) # positivity constraint
...
def trainWANCO(model,device,params,optimizer,scheduler,model_2,optimizer_2,scheduler_2):
    ...
    # Visualization of the initial model
    ...
    for step in range(params["trainStep"]):
      ...
      lambda_value = model_2(xxx_2).reshape(-1,1) # adversarial term
      lambda_dict = {}
      for i in range(params["dd"]):
...
# function used to compute and plot the final partition
...
def main():
    ...
    params["lr"] = 0.006 # Learning rate
    params["lr2"] = 0.032 
    params["width"] = 100 # Width of primal network 
    params["depth"] = 3 # number of hidden layer of primal network
    params["trainStep"] = 40000
    params["penalty"] = 900000  # beta in paper
    params["alpha"] = 1.0003 # amplifier of beta
    params["volumn"] = 1 # integration of u_i
    params["penalty2"] = 100 # C_0 in paper
    params["writeStep"] = 50
