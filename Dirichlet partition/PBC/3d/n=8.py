...
def trainWANCO(model,device,params,optimizer,scheduler,model_2,optimizer_2,scheduler_2):
    ...
    data[:,0:3] = torch.from_numpy(XDE_sampler.sample(num_samples, num_dimensions, sampler= 'Hammersley'))
    ...
    for step in range(params["trainStep"]):
      ...
      for i in range(params["dd"]):
        ...
        gradient_norm_2_dict[f"gradient_u{i}_norm2"] = gradient_u_dict[f"du{i}dxy"][:,0]**2 + gradient_u_dict[f"du{i}dxy"][:,1]**2 + gradient_u_dict[f"du{i}dxy"][:,2]**2
...
def main():
    ...
    params["milestone"] = [500,1500,3000,6000,9000,11000,13000,15000,17000,19000,25000,35000,45000,55000,60500,75000,85000,95000]
    params["gamma"] = 0.5
    params["decay"] = 0.0001
