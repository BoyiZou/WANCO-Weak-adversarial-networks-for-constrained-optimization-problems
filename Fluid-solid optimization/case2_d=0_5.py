...
class ResNet(torch.nn.Module):
    ...
    def forward(self, x, params,number_of_pts,device):
        ...
        x[:,2] = torch.relu(x[:,2]) # positivity constraint
...
def trainWANCO(model,device,params,optimizer,scheduler,model_1,optimizer_1,scheduler_1,model_2,optimizer_2,scheduler_2,model_3,optimizer_3,scheduler_3):
    ...
    for step in range(params["trainStep"]):
        ...
        if step%params["plotStep"] == params["plotStep"]-1:
          ...
          plt.figure()
          plt.imshow(Train_u1.T,cmap = 'viridis',origin='lower',aspect=3/2)
          cb = plt.colorbar(shrink=0.7)
          plt.show()

          plt.imshow(Train_u2.T,cmap = 'viridis',origin='lower',aspect=3/2)
          cb = plt.colorbar(shrink=0.7)
          plt.show()


          plt.imshow(Train_phi.T,cmap = 'viridis',origin='lower',aspect=3/2)
          cb = plt.colorbar(shrink=0.7)
          plt.show()
          ...
          plt.figure()
          plt.imshow(phase_fluid.T,cmap = 'viridis',origin='lower',aspect=3/2)
          cb = plt.colorbar(shrink=0.7)
          plt.show()

          plt.imshow((Train_u2**2+Train_u1**2).T**0.5,cmap = 'viridis',origin='lower',aspect=3/2)
          cb = plt.colorbar(shrink=0.7)
          plt.show()
...
def main():
    ...
    optimizer_3 = torch.optim.Adam(model_3.parameters(),lr=params["lr"],weight_decay=params["decay"])
    scheduler_3 = MultiStepLR(optimizer_3,milestones=params["milestone"],gamma=params["gamma"])

    print("Generating network costs %s seconds."%(time.time()-startTime))

    startTime = time.time()
    trainWANCO(model,device,params,optimizer,scheduler,model_1,optimizer_1,scheduler_1,model_2,optimizer_2,scheduler_2,model_3,optimizer_3,scheduler_3)  #trainWANCO model
