...
def trainWANCO(model,device,params,optimizer,scheduler,model_2,optimizer_2,scheduler_2):
    ...
    for step in range(params["trainStep"]):
        ...
        loss = params["penalty2"]*(1/2*torch.mean(grad_u_2norm_2) + (1/(params["epsilon"]**2))*torch.mean((uxt**2-1)**2))  + params["penalty"]*(mass)**2
...
def main():
    ...
    params["milestone"] = [3000,5000,6500,8000,13500,14000,15000,16500,18000,10000,12000,18000,20000,21000,23000,24000,25000,26000,27000,28000,29000,31000,33000,35000,37000,39000,48000]
    params["gamma"] = 0.5
    params["decay"] = 0.0001
